...
#Read in the 12 files containing the monthly values from 1980_2023
...
for mon in month: #loop through the files; the MAR melting data and the AR indicies 
	...
	file1 = "/home/jwille/xenon_data/IGE_data/vIVT_IWV/ERA5/coarse/80_23/IWV/lat_rst_ant_IWV_ERA5_2DIVT_"+mon+"_80_23.nc"


#Read in data

	fh1 = Dataset(file1, mode='r',format="NETCDF4")
	lat_test = fh1.variables['lat'][::-1]  
	lon_test = fh1.variables['lon'][:]  
	iwv_full = fh1.variables['IWV'][:,::-1,:]
	iwv = iwv_full[:,14:111,:]   #The latitude range of consideration is from -37 to -85



	time_plot = fh1.variables['time'][:]
	units=fh1.variables['time'].units
	ntime = time_plot.size
	...
	# Attribute doesnt exist
		cal = 'standard' 
	datevar=(nc.num2date(time_plot,units=units,calendar=cal))
	fh1.close()
...
#Run the algorithm
	...
	lon_index_list = []
	landfall_idx = []
	iwv_per  = np.percentile(iwv, 98, axis=0) #Calc the 98th percentile of viwv at all points
	...
	np.save('/home/jwille/xenon_data/IGE_data/ar_catalogues/era5/coord_idx/'+mon+'_80_23_ant_iwv_lat_idx_v2.4',lat_index_list)
	np.save('/home/jwille/xenon_data/IGE_data/ar_catalogues/era5/coord_idx/'+mon+'_80_23_ant_iwv_lon_idx_v2.4',lon_index_list)
